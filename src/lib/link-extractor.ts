...
export async function extractProductMetadata(url: string): Promise<ExtractionResult> {
  ...
  try {
    ...
    if (response.ok) {
      ...
      // Clean Amazon title prefixes/suffixes
      ...
      // 4. Pricing (heuristics from meta tags or JSON-LD)
      let price = 0;
      let origPrice = 0;
        ...
        try {
          ...
          if (json['@type'] === 'Product' || json['@type'] === 'IndividualProduct') {
            ...
            if (json.image && !extracted.image_url) {
              extracted.image_url = Array.isArray(json.image) ? json.image[0] : json.image;
            }
            if (json.description && !extracted.description) extracted.description = json.description;
            if (json.offers) {
      ...
      if (price > 0) {
        extracted.current_price = Math.round(price);
        // Estimate realistic MRP if not explicitly provided
        origPrice = Math.round(price * 1.8);
        extracted.original_price = origPrice;
      }

      // Infer category from title or breadcrumbs
      ...
      } else if (titleLower.includes('jacket') || titleLower.includes('bomber')) {
      ...
      extractionNote = 'Marketplace protected or anti-bot triggered; fallback template loaded for review.';
    }
  } catch (err) {
...
function inferTitleFromUrl(url: URL, marketplace: string): string {
  ...
  if (segments.length > 0) {
    const slug = segments[0] === 'dp' && segments[1] ? segments[1] : segments[0];
    const cleaned = slug.replace(/[-_]+/g, ' ').replace(/\.html?$/i, '');
    if (cleaned.length > 4 && isNaN(Number(cleaned))) {
